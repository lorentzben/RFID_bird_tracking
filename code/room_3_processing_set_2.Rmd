...
for(i in 1:length(bird_ids_room_3)){
  ...
  res <- sep_bird_id_xts(samp_name="room 3",cage_obj =room_3, samp_id = bird_ids_room_3[i])
  reslist_room_3[[i]] <- res
  print(bird_ids_room_3[[i]])
...
# TODO update with birds that do not meet cutoff
remove <- c("6879","6912","6915","6948","6953","6968","6987","9008","9029")
...
for(i in 1:length(bird_ids_room_3_new)){
  res <- sep_bird_id_period(samp_name="room 3",cage_obj =room_3, samp_id = bird_ids_room_3_new[i],cutoff = "2021-03-09 T21:00:00/2021-05-07 T14:00:00")
...
whole_data_sanity <- reslist2_room_3[[1]]$daily_obj
for(item in reslist2_room_3){
...
for(i in 1:length(reslist2_room_3)){
  ...
  daily_trans_table <- calc_trans_period(reslist2_room_3[[i]]$ID,reslist2_room_3[[i]]$daily_obj,reslist2_room_3[[i]]$xts_obj,"d")
  weekly_trans_table <- calc_trans_period(reslist2_room_3[[i]]$ID,reslist2_room_3[[i]]$weekly_obj,reslist2_room_3[[i]]$xts_obj,"w")
  monthly_trans_table <- calc_trans_period(reslist2_room_3[[i]]$ID,reslist2_room_3[[i]]$monthly_obj,reslist2_room_3[[i]]$xts_obj,"m")
...
for(i in 1:length(reslist2_room_3)){
  ...
  minute_trans_table <- calc_trans_period(reslist2_room_3[[i]]$ID, minute_period, reslist2_room_3[[i]]$xts_obj, "Hr")  
...
#length(unique(room3_daily$ID)) nrow=31 comes from 31 birds used
tmp2 <- matrix(as.vector(as.numeric(tmp[,2])), nrow=31, byrow=T)
...
tmp <- cbind(as.character(room3_daily$ID), as.numeric(room3_daily$total))
#nrow=32 comes from 31 birds used
tmp2 <- matrix(as.vector(as.numeric(tmp[,2])), nrow=31, byrow=T)
total_totals <- rowSums(tmp2)
...
p
ggsave(p, filename = "../figures/most_active_bird_pca_rm3_set_2.png",device = "png",width = unit(7,"in"),height=unit(7,"in"))
...
most_active_grp <- data.frame(x = most_active_intra_day$day, y = c(most_active_intra_day$bottom, most_active_intra_day$mid, most_active_intra_day$top), group= c(rep("Bottom",nrow(most_active_intra_day)),rep("Mid",nrow(most_active_intra_day)),rep("Top",nrow(most_active_intra_day))))
 ...
 q <- ggplot(most_active_grp, aes(x,y,col=group))+
  ...
  theme_bw()
q
ggsave(q, filename = "../figures/most_active_transitions_per_day_rm3_set_2.png",device="png")
```
...
p <- ggplot(pr.feb$x[,1:2], aes(x=PC1, y=PC2, color=as.character(least_active_intra_day$cluster)))+
  ...
  theme(legend.position = "bottom", legend.direction = "vertical",legend.text = element_text(size=8))
p
ggsave(p, filename = "../figures/least_active_bird_pca_rm3_set_2.png",device = "png",width = unit(7.5,"in"),height=unit(7.5,"in"))
...
q <-ggplot(least_active_grp, aes(x,y,col=group))+
  ...
  theme_bw()
q
ggsave(q, filename="../figures/least_active_transitions_per_day_rm3_set_2.png",device="png")
```
...
cluster_table
write.csv(cluster_table, "../output/organized_birds_by_activity_daily_rm3_set_2.csv",row.names = F)
```
...
ggsave(q, filename="../figures/low_active_transitions_rm3_set_2.png",device="png")
```
...
q <-ggplot(med_act_grp, aes(x,y,col=group))+
  geom_line() +
  facet_grid(factor(group,levels=c("Top","Mid","Bottom"))~ .) +
  labs(title="Medium Activity Birds Transitions in Room 3", color="Legend")+
  scale_x_continuous(name="Days",n.breaks = 14)+
  scale_y_continuous(name="Transitions") +
  scale_color_manual(values=c("Top"="royalblue","Mid"="tomato","Bottom" = "seagreen")) +
  theme_bw()
...
ggsave(q, filename="../figures/med_active_transitions_rm3_set_2.png",device="png")
```
...
ggsave(q, filename="../figures/high_active_transitions_rm3_set_2.png",device="png")
```
...
for(i in 1:length(minute_reslist_room3_hr)){
  minute_trans_tab <- calc_trans_period_duration(reslist2_room_3[[i]]$ID, minute_reslist_room3_hr[[i]],reslist2_room_3[[i]]$xts_obj, "hr")
  #minute_trans_table <- calc_trans_period_duration(reslist2_6928[[i]]$ID, minute_period, reslist2_6928[[i]]$xts_obj, "Hr")
  result <- list("ID"=reslist2_room_3[[i]]$ID,"hour"=minute_trans_tab,"raw"=reslist2_room_3[[i]]$xts_obj)
