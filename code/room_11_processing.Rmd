...
p
ggsave(p, filename = "../figures/most_active_bird_pca_rm11.png",device = "png",width = unit(7,"in"),height=unit(7,"in"))
...
most_active_grp <- data.frame(x = most_active_intra_day$day, y = c(most_active_intra_day$bottom, most_active_intra_day$mid, most_active_intra_day$top), group= c(rep("Bottom",nrow(most_active_intra_day)),rep("Mid",nrow(most_active_intra_day)),rep("Top",nrow(most_active_intra_day))))
 ...
 q <- ggplot(most_active_grp, aes(x,y,col=group))+
  ...
  theme_bw()
q
ggsave(q, filename = "../figures/most_active_transitions_per_day_rm11.png",device="png")
```
...
p <- ggplot(pr.feb$x[,1:2], aes(x=PC1, y=PC2, color=as.character(least_active_intra_day$cluster)))+
  ...
  theme(legend.position = "bottom", legend.direction = "vertical",legend.text = element_text(size=8))
p
ggsave(p, filename = "../figures/least_active_bird_pca_rm11.png",device = "png",width = unit(7.5,"in"),height=unit(7.5,"in"))
...
q <-ggplot(least_active_grp, aes(x,y,col=group))+
  ...
  theme_bw()
q
ggsave(q, filename="../figures/least_active_transitions_per_day_rm11.png",device="png")
```
...
ggsave(q, filename="../figures/low_active_transitions_rm11.png",device="png")
```
...
ggsave(q, filename="../figures/med_active_transitions_rm11.png",device="png")
```
...
ggsave(q, filename="../figures/high_active_transitions_rm11.png",device="png")
```
...
ggsave(q, filename="../figures/duration_transitions_rm11.png",device="png")
ggsave(z, filename="../figures/duration_transitions_day_rm11.png",device="png")
```
