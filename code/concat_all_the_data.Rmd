...
cutoff_6988 <- data.frame()
for(i in 1:length(reslist_6988)){
  id <- reslist_6988[[i]]$ID
  top <- index(head(reslist_6988[[i]]$xts_obj))[1]
  bottom <- index(tail(reslist_6988[[i]]$xts_obj))[6]
  rec <- cbind(id,as.character(top),as.character(bottom))
  cutoff_6988 <- rbind(cutoff_6988,rec)
...
for(i in 1:length(bird_ids_6988_new)){
  res <- sep_bird_id_period(samp_name="6988",cage_obj =six_nine_eight_eight, samp_id = bird_ids_6988_new[i],cutoff = "2021-02-01 T20:00:00/2021-05-07 T15:00:00")
  reslist2_6988[[i]] <- res
}
...
for(i in 2:length(reslist2_6988)){
  ...
  colnames(current) <- as.character(reslist2_6988[[i]]$ID)
...
cutoff_9013 <- data.frame()
for(i in 1:length(reslist_9013)){
  id <- reslist_9013[[i]]$ID
  top <- index(head(reslist_9013[[i]]$xts_obj))[1]
  bottom <- index(tail(reslist_9013[[i]]$xts_obj))[6]
  rec <- cbind(id,as.character(top),as.character(bottom))
  cutoff_9013 <- rbind(cutoff_9013,rec)
...
for(i in 1:length(bird_ids_9013_new)){
  res <- sep_bird_id_period(samp_name="9013",cage_obj =nine_zero_one_three, samp_id = bird_ids_9013_new[i],cutoff = "2021-02-01 T20:00:00/2021-05-07 T15:00:00")
  reslist2_9013[[i]] <- res
}
...
cutoff_6928 <- data.frame()
for(i in 1:length(reslist_6928)){
  id <- reslist_6928[[i]]$ID
  top <- index(head(reslist_6928[[i]]$xts_obj))[1]
  bottom <- index(tail(reslist_6928[[i]]$xts_obj))[6]
  rec <- cbind(id,as.character(top),as.character(bottom))
  cutoff_6928 <- rbind(cutoff_6928,rec)
...
#can remove 6936 and 6949 if needed
...
for(i in 1:length(bird_ids_6928_new)){
  res <- sep_bird_id_period(samp_name="6928",cage_obj =six_nine_two_eight, samp_id = bird_ids_6928_new[i],cutoff = "2021-02-01 T20:00:00/2021-05-07 T15:00:00")
  reslist2_6928[[i]] <- res
}
...
cutoff_6984 <- data.frame()
for(i in 1:length(reslist_6894)){
  id <- reslist_6894[[i]]$ID
  top <- index(head(reslist_6894[[i]]$xts_obj))[1]
  bottom <- index(tail(reslist_6894[[i]]$xts_obj))[6]
  rec <- cbind(id,as.character(top),as.character(bottom))
  cutoff_6984 <- rbind(cutoff_6984,rec)
  
}
cutoff_6984

# old values "6862","6880","6881","9023","9038","9042","9050","9059"
#remove <- c("9023","9038","9042","9050","9059","6862","6880","6881")
remove <- c("9023","6862","6880","6881","6882","6887","6891","6892","6893","6894","6900")
bird_ids_6894_new <- bird_ids_6894 [! bird_ids_6894  %in% remove ]
#generate list of the first 10 truncated XTS objects
reslist2_6894 <- list()
for(i in 1:length(bird_ids_6894_new)){
  res <- sep_bird_id_period(samp_name="6894",cage_obj =six_eight_nine_four, samp_id = bird_ids_6894_new[i], cutoff="2021-02-01 T20:00:00/2021-05-07 T15:00:00")
  reslist2_6894[[i]] <- res
}
...
for(i in 1:length(reslist2_6988)){
  ...
  daily_trans_table <- calc_trans_period(reslist2_6988[[i]]$ID,reslist2_6988[[i]]$daily_obj,reslist2_6988[[i]]$xts_obj,"d", 0)
  weekly_trans_table <- calc_trans_period(reslist2_6988[[i]]$ID,reslist2_6988[[i]]$weekly_obj,reslist2_6988[[i]]$xts_obj,"w", 0)
  monthly_trans_table <- calc_trans_period(reslist2_6988[[i]]$ID,reslist2_6988[[i]]$monthly_obj,reslist2_6988[[i]]$xts_obj,"m",0)
...
for(i in 1:length(reslist2_9013)){
  ...
  daily_trans_table <- calc_trans_period(reslist2_9013[[i]]$ID,reslist2_9013[[i]]$daily_obj,reslist2_9013[[i]]$xts_obj,"d",0)
  weekly_trans_table <- calc_trans_period(reslist2_9013[[i]]$ID,reslist2_9013[[i]]$weekly_obj,reslist2_9013[[i]]$xts_obj,"w",0)
  monthly_trans_table <- calc_trans_period(reslist2_9013[[i]]$ID,reslist2_9013[[i]]$monthly_obj,reslist2_9013[[i]]$xts_obj,"m",0)
...
for(i in 1:length(reslist2_6928)){
  ...
  daily_trans_table <- calc_trans_period(reslist2_6928[[i]]$ID,reslist2_6928[[i]]$daily_obj,reslist2_6928[[i]]$xts_obj,"d",0)
  weekly_trans_table <- calc_trans_period(reslist2_6928[[i]]$ID,reslist2_6928[[i]]$weekly_obj,reslist2_6928[[i]]$xts_obj,"w",0)
  monthly_trans_table <- calc_trans_period(reslist2_6928[[i]]$ID,reslist2_6928[[i]]$monthly_obj,reslist2_6928[[i]]$xts_obj,"m",0)
...
for(i in 1:length(reslist2_6894)){
  ...
  result <- list("ID"=reslist2_6894[[i]]$ID,"daily"=daily_trans_table, "weekly"=weekly_trans_table,"monthly"=monthly_trans_table, "raw"=reslist2_6894[[i]]$xts_obj)
...
p
ggsave(p, filename = paste0(as.character(most_active[j]),"_most_active_bird_pca.png"),device = "png",width = unit(7,"in"),height=unit(7,"in"))
...
most_active_grp <- data.frame(x = as.Date(most_active_intra_day$day), y = c(most_active_intra_day$bottom, most_active_intra_day$mid, most_active_intra_day$top), group= c(rep("Bottom",nrow(most_active_intra_day)),rep("Mid",nrow(most_active_intra_day)),rep("Top",nrow(most_active_intra_day))))
 ...
 q <- ggplot(most_active_grp, aes(x,y,col=group))+
  ...
  scale_y_continuous(name="Transitions Into",n.breaks = 8,limits = c(0,40)) +
  scale_color_manual(values=c("Top"="royalblue","Mid"="tomato","Bottom" = "seagreen")) +
  theme_bw()
  
q

ggsave(q, filename = paste0(as.character(most_active[j]),"_most_active_transitions_per_day.png"),device="png")
}
```
...
p <- ggplot(pr.feb$x[,1:2], aes(x=PC1, y=PC2, color=as.character(least_active_intra_day$cluster)))+
  ...
  theme(legend.position = "bottom", legend.direction = "vertical",legend.text = element_text(size=8))
p
ggsave(p, filename = "least_active_bird_pca.png",device = "png",width = unit(7.5,"in"),height=unit(7.5,"in"))
...
q <-ggplot(least_active_grp, aes(x,y,col=group))+
  ...
  theme_bw()
q
ggsave(q, filename="least_active_transitions_per_day.png",device="png")
```
...
cluster_table
write.csv(cluster_table, "organized_birds_by_activity_daily.csv",row.names = F)
```
...
low_act_grp <- data.frame(x = as.Date(low_act_agg_tab$day), y = c(low_act_agg_tab$bottom, low_act_agg_tab$mid, low_act_agg_tab$top), group= c(rep("Bottom",nrow(low_act_agg_tab)),rep("Mid",nrow(low_act_agg_tab)),rep("Top",nrow(low_act_agg_tab))))
...
q <-ggplot(low_act_grp, aes(x,y,col=group))+
  geom_line() +
  facet_grid(factor(group,levels=c("Top","Mid","Bottom"))~ .) +
  labs(title=paste0("Low Activity Birds Transitions (",round((length(low_act_bird_ids)/length(unique(daily$ID)))*100,2),"%; n=",length(unique(daily$ID)),")"), color="Legend")+
  scale_x_date(name="Weeks",breaks = seq(min(low_act_grp$x),max(low_act_grp$x),by="week"),labels = as.numeric(format(seq(min(low_act_grp$x),max(low_act_grp$x),by="week"),"%U"))+23)+
  scale_y_continuous(name="Average Transitions Into",n.breaks = 8,limits = c(0,30)) +
  scale_color_manual(values=c("Top"="royalblue","Mid"="tomato","Bottom" = "seagreen")) +
  theme_bw()
q
print("which birds are in this plot?")
print(low_act_bird_ids)
ggsave(q, filename="low_active_transitions_no_rm3.png",device="png")
```
...
# plot the low activity birds
med_act_grp <- data.frame(x = as.Date(med_act_agg_tab$day), y = c(med_act_agg_tab$bottom, med_act_agg_tab$mid, med_act_agg_tab$top), group= c(rep("Bottom",nrow(med_act_agg_tab)),rep("Mid",nrow(med_act_agg_tab)),rep("Top",nrow(med_act_agg_tab))))
...
q <-ggplot(med_act_grp, aes(x,y,col=group))+
  geom_line() +
  facet_grid(factor(group,levels=c("Top","Mid","Bottom"))~ .) +
  labs(title=paste0("Medium Activity Birds Transitions (",round((length(med_act_bird_ids)/length(unique(daily$ID)))*100,2),"%; n=",length(unique(daily$ID)),")"), color="Legend")+
  scale_x_date(name="Weeks",breaks = seq(min(med_act_grp$x),max(med_act_grp$x),by="week"),labels = as.numeric(format(seq(min(med_act_grp$x),max(med_act_grp$x),by="week"),"%U"))+23)+
  scale_y_continuous(name="Average Transitions Into",n.breaks = 8,limits = c(0,30)) +
  scale_color_manual(values=c("Top"="royalblue","Mid"="tomato","Bottom" = "seagreen")) +
  theme_bw()
q
...
ggsave(q, filename="med_active_transitions_no_rm3.png",device="png")
```
...
# plot the low activity birds
high_act_grp <- data.frame(x = as.Date(high_act_agg_tab$day), y = c(high_act_agg_tab$bottom, high_act_agg_tab$mid, high_act_agg_tab$top), group= c(rep("Bottom",nrow(high_act_agg_tab)),rep("Mid",nrow(high_act_agg_tab)),rep("Top",nrow(high_act_agg_tab))))
...
q <-ggplot(high_act_grp, aes(x,y,col=group))+
  geom_line() +
  facet_grid(factor(group,levels=c("Top","Mid","Bottom"))~ .) +
  labs(title=paste0("High Activity Birds Transitions (",round((length(high_act_bird_ids)/length(unique(daily$ID)))*100,2),"%; n=",length(unique(daily$ID)),")"), color="Legend")+
  scale_x_date(name="Weeks",breaks = seq(min(high_act_grp$x),max(high_act_grp$x),by="week"),labels = as.numeric(format(seq(min(high_act_grp$x),max(high_act_grp$x),by="week"),"%U"))+23)+
  scale_y_continuous(name="Average Transitions Into",n.breaks = 8,limits = c(0,30)) +
  scale_color_manual(values=c("Top"="royalblue","Mid"="tomato","Bottom" = "seagreen")) +
  theme_bw()
q
...
ggsave(q, filename="high_active_transitions_no_rm3.png",device="png")
```
...
for(i in 1:length(reslist2_6988)){
  ...
  minute_trans_table <- calc_trans_period(reslist2_6988[[i]]$ID, minute_period, reslist2_6988[[i]]$xts_obj, "Hr",0)  
...
for(i in 1:length(reslist2_9013)){
  ...
  minute_trans_table <- calc_trans_period(reslist2_9013[[i]]$ID, minute_period, reslist2_9013[[i]]$xts_obj, "Hr",0)  
...
trans_reslist_room8_min <- list()

for(i in 1:length(reslist2_6928)){
  minute_period <- to_minute(reslist2_6928[[i]]$xts_obj$subzone)
  minute_trans_table <- calc_trans_period(reslist2_6928[[i]]$ID, minute_period, reslist2_6928[[i]]$xts_obj, "Hr",0)  
  result <- list("ID"=reslist2_6928[[i]]$ID,"hour"=minute_trans_table, "raw"=reslist2_6928[[i]]$xts_obj)
  trans_reslist_room8_min[[i]] <- result
}

room8_hour <- data.frame()

for(item in trans_reslist_room8_min){
  room8_hour <- rbind(room8_hour, item$hour[c("sample","bottom","mid","top","total")])
}
...
for(i in 1:length(minute_reslist_room2_hr)){
  minute_trans_tab <- calc_trans_period_duration(reslist2_6988[[i]]$ID, minute_reslist_room2_hr[[i]],reslist2_6988[[i]]$xts_obj, "hr",0)
...
ggsave(q, filename="duration_transitions_rm2.png",device="png")
ggsave(z, filename="duration_transitions_day_rm2.png",device="png")
```

```{r}

for (i in 1:length(trans_reslists)){
  write.csv(trans_reslists[[i]]$daily, paste0("transition_tabs/",trans_reslists[[i]]$ID,"_daily.csv"),row.names = F)
  write.csv(trans_reslists[[i]]$raw, paste0("transition_tabs/",trans_reslists[[i]]$ID,"_raw.csv"), row.names = F)
}
