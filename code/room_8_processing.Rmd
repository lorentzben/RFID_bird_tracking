...
for(i in 1:length(bird_ids_6928_new)){
  res <- sep_bird_id_period(samp_name="6928",cage_obj =six_nine_two_eight, samp_id = bird_ids_6928_new[i],cutoff = "2021-02-05 T11:30:00/2021-05-06 T23:00:00")
  reslist2_6928[[i]] <- res
}
...
whole_data_sanity <- reslist2_6928[[1]]$daily_obj
...
tmp2 <- matrix(as.vector(as.numeric(tmp[,2])), nrow=20, byrow=T)
...
p
ggsave(p, filename = "../figures/most_active_bird_pca_rm8.png",device = "png",width = unit(7,"in"),height=unit(7,"in"))
...
most_active_grp <- data.frame(x = most_active_intra_day$day, y = c(most_active_intra_day$bottom, most_active_intra_day$mid, most_active_intra_day$top), group= c(rep("Bottom",nrow(most_active_intra_day)),rep("Mid",nrow(most_active_intra_day)),rep("Top",nrow(most_active_intra_day))))
 ...
 q <- ggplot(most_active_grp, aes(x,y,col=group))+
  ...
  theme_bw()
q
ggsave(q, filename = "../figures/most_active_transitions_per_day_rm8.png",device="png")
```
...
p <- ggplot(pr.feb$x[,1:2], aes(x=PC1, y=PC2, color=as.character(least_active_intra_day$cluster)))+
  ...
  theme(legend.position = "bottom", legend.direction = "vertical",legend.text = element_text(size=8))
p
ggsave(p, filename = "../figures/least_active_bird_pca_rm8.png",device = "png",width = unit(7.5,"in"),height=unit(7.5,"in"))
...
q <-ggplot(least_active_grp, aes(x,y,col=group))+
  ...
  theme_bw()
q
ggsave(q, filename="../figures/least_active_transitions_per_day_rm8.png",device="png")
```
...
cluster_table
write.csv(cluster_table, "../output/organized_birds_by_activity_daily_rm8.csv",row.names = F)
```
...
ggsave(q, filename="../figures/low_active_transitions_rm8.png",device="png")
```
...
ggsave(q, filename="../figures/med_active_transitions_rm8.png",device="png")
```
...
ggsave(q, filename="../figures/high_active_transitions_rm8.png",device="png")
```
...
for(i in 1:length(minute_reslist_room8_hr)){
  minute_trans_tab <- calc_trans_period_duration(reslist2_6928[[i]]$ID, minute_reslist_room8_hr[[i]],reslist2_6928[[i]]$xts_obj, "hr")
...
ggsave(q, filename="../figures/duration_transitions_rm8.png",device="png")
ggsave(z, filename="../figures/duration_transitions_day_rm8.png",device="png")
```
