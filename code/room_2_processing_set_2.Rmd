...
#length(unique(room2_daily$ID)) nrow=37 comes from 37 birds used
tmp2 <- matrix(as.vector(as.numeric(tmp[,2])), nrow=37, byrow=T)
...
cluster_table
write.csv(cluster_table, "../output/organized_birds_by_activity_daily_rm2_set_2.csv",row.names = F)
```
...
ggsave(q, filename="../figures/low_active_transitions_rm2_set_2.png",device="png")
```
...
ggsave(q, filename="../figures/med_active_transitions_rm2_set_2.png",device="png")
```
...
ggsave(q, filename="../figures/high_active_transitions_rm2_set_2.png",device="png")
```
...
for(i in 1:length(minute_reslist_room2_hr)){
  minute_trans_tab <- calc_trans_period_duration(reslist2_room_2[[i]]$ID, minute_reslist_room2_hr[[i]],reslist2_room_2[[i]]$xts_obj, "hr")
  #minute_trans_table <- calc_trans_period_duration(reslist2_6928[[i]]$ID, minute_period, reslist2_6928[[i]]$xts_obj, "Hr")
  result <- list("ID"=reslist2_room_2[[i]]$ID,"hour"=minute_trans_tab,"raw"=reslist2_room_2[[i]]$xts_obj)
