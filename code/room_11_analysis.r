...
room_11["DateTime"] <- as.POSIXct(room_11$access, origin="1970-01-01", tz="UTC")
...
room_11$accessdate <- ymd_hms(room_11$DateTime)
...
room_11_all_analysis <- room_11_struct |>
 mutate(slicedTsibble = map(tsibble, ~ slice_tsibble(as_tibble(.x), "2021-03-10 T04:00:00", "2021-05-06 T22:00:00")))
...
room_11_all_room_time_budget |>
 ...
 write.csv(row.names=F, '../output/all_rooms/room_11_all_room_time_budget.csv')
...
# All Room room_11 Daily Time Budget Analysis
...
room_11_all_room_day <- room_11_all_room_day |>
...
# Run getTimeBudgetProp for each daily interval tables
 ...
 room_11_all_room_time_budget <- room_11_all_room_day |>
...
for(i in 1:length(room_11_all_room_time_budget$tagname)){

    current_tag <- room_11_all_room_time_budget$tagname[i]

    current_day_tb<- room_11_all_room_time_budget |>
        slice(i) |> 
        pull(daily_tb) |> 
        pluck(1)
    ...
    write.csv(current_day_tb_df, paste0("../intermediate/all_rooms/room_11_day_time_budget_",current_tag,".csv"),row.names=F)

    current_night_tb <- room_11_all_room_time_budget |>
        slice(i) |> 
        pull(night_tb) |> 
        pluck(1)
    ...
    write.csv(current_night_tb_df, paste0("../intermediate/all_rooms/room_11_night_time_budget_",current_tag,".csv"),row.names=F)
}
...
datebreaks <- seq(as.Date(as.POSIXct.numeric(head(unique(sb_data$interval1),n=1),origin="1970-01-01", tz="UTC")), as.Date(as.POSIXct.numeric(tail(unique(sb_data$interval1),n=1),origin="1970-01-01", tz="UTC")), by="7 days")

datebreaks <- c(datebreaks, as.Date(as.POSIXct.numeric(tail(unique(sb_data$interval1),n=1),origin="1970-01-01", tz="UTC")))

all_datebreak <- seq(as.Date(as.POSIXct.numeric(head(unique(sb_data$interval1),n=1),origin="1970-01-01", tz="UTC")), as.Date(as.POSIXct.numeric(tail(unique(sb_data$interval1),n=1),origin="1970-01-01", tz="UTC")), by="1 days")



day_3_sb_plot <- ggplot(data = sb_data, aes(x = as.Date(as.POSIXct.numeric(as.numeric(interval1),origin="1970-01-01", tz="UTC")), y=values, fill=ind)) + 
...
ggsave(paste0("../figures/all_day/room11/day_daily_time_budget_stack_bar_for_", nest_day_tbs[i,1],".png"), day_3_sb_plot)
}
...
ggsave(paste0("../figures/all_day/room11/night_daily_time_budget_stack_bar_for_", nest_day_tbs[i,1],".png"), day_3_sb_plot)
}
...
# TODO  Check Limits
room_11_sb_plot <- ggplot(data = day_flat, aes(x = as.Date(as.POSIXct.numeric(as.numeric(interval1),origin="1970-01-01", tz="UTC")), y=values, fill=ind)) + 
geom_bar(stat="identity") +
theme_bw() +  
xlab("Day of Study") + 
ylab("Prop. of Time Spent in Zone") +
...
room_11_sb_night_plot <- ggplot(data = night_flat, aes(x =as.Date(as.POSIXct.numeric(as.numeric(interval1),origin="1970-01-01", tz="UTC")), y=values, fill=ind)) + 
geom_bar(stat="identity") +
theme_bw() +  
xlab("Day of Study") + 
ylab("Prop. of Time Spent in Zone") +
...
for(i in 1:length( room_11_interval$tagname)){

    current_tag <- room_11_interval$tagname[i]

    current_tsibble <- room_11_interval |>
        slice(i) |> 
        pull(interval) |> 
        pluck(1)
    ...
    write.csv(current_tsibble, paste0("../intermediate/all_rooms/overall_interval/room_11_interval_",current_tag,".csv"),row.names=F)
}

# Daily 

room_11_day_int <- room_11_all_room_day |>
  mutate(daily_int = map(day, ~time_to_intervals(.x)))

for(i in 1:length(room_11_day_int$tagname)){

    current_tag <- room_11_day_int$tagname[i]

    current_day_tsibble <- room_11_day_int |>
        slice(i) |> 
        pull(daily_int) |> 
        pluck(1)
    ...
    write.csv(current_day_tsibble, paste0("../intermediate/all_rooms/daily_interval/room_11_interval_",current_tag,".csv"),row.names=F)
}

# Nightly

room_11_night_int <- room_11_all_room_day |>
  mutate(nightly_int = map(night, ~time_to_intervals(.x)))

for(i in 1:length(room_11_night_int$tagname)){

    current_tag <- room_11_night_int$tagname[i]

    current_night_tsibble <- room_11_night_int |>
        slice(i) |> 
        pull(nightly_int) |> 
        pluck(1)
    ...
    write.csv(current_night_tsibble, paste0("../intermediate/all_rooms/nightly_interval/room_11_interval_",current_tag,".csv"),row.names=F)
}

### END OF ROOM 11 ANALYSIS ###