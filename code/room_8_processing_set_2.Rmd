...
for(i in 1:length(bird_ids_room_8)){
  ...
  res <- sep_bird_id_xts(samp_name="room 9",cage_obj =room_8, samp_id = bird_ids_room_8[i])
  reslist_room_8[[i]] <- res

  print(bird_ids_room_8[[i]])
...
for(i in 1:length(bird_ids_room_8_new)){
  res <- sep_bird_id_period(samp_name="room 8",cage_obj =room_8, samp_id = bird_ids_room_8_new[i],cutoff = "2021-02-05 T12:00:00/2021-05-06 T23:00:00")
...
whole_data_sanity <- reslist2_room_8[[1]]$daily_obj
for(item in reslist2_room_8){
...
for(i in 1:length(reslist2_room_8)){
  ...
  daily_trans_table <- calc_trans_period(reslist2_room_8[[i]]$ID,reslist2_room_8[[i]]$daily_obj,reslist2_room_8[[i]]$xts_obj,"d")
  weekly_trans_table <- calc_trans_period(reslist2_room_8[[i]]$ID,reslist2_room_8[[i]]$weekly_obj,reslist2_room_8[[i]]$xts_obj,"w")
  monthly_trans_table <- calc_trans_period(reslist2_room_8[[i]]$ID,reslist2_room_8[[i]]$monthly_obj,reslist2_room_8[[i]]$xts_obj,"m")
...
write.csv(room_8_daily,"../output/room_8_daily_intra.csv",row.names = F)
write.csv(room_8_weekly,"../output/room_8_weekly_intra.csv",row.names = F)
write.csv(room_8_monthly,"../output/room_8_monthly_intra.csv",row.names = F)

```

# Room 8 hourly dataset

```{r room 8 hourly dataset}

trans_reslist_room8_min <- list()

for(i in 1:length(reslist2_room_8)){
  minute_period <- to_minute(reslist2_room_8[[i]]$xts_obj$subzone)
  minute_trans_table <- calc_trans_period(reslist2_room_8[[i]]$ID, minute_period, reslist2_room_8[[i]]$xts_obj, "Hr")  
...
## Determine which bird is the most active in the Room 8 Dataset

```{r who is the most active?}
room8_daily <- data.frame()
room8_weekly <- data.frame()
room8_monthly <- data.frame()
#TODO select sample as opposed to ID for intra bird comparison (alpha eq)
for(item in trans_reslist_room_8){
  room8_daily <- rbind(room8_daily, item$daily[c("ID","bottom","mid","top","total")])
  room8_weekly <- rbind(room8_weekly, item$weekly[c("ID","bottom","mid","top","total")])
  room8_monthly <- rbind(room8_monthly, item$monthly[c("ID","bottom","mid","top","total")])
}
tmp <- cbind(as.character(room8_daily$ID), as.numeric(room8_daily$total))
#length(unique(room8_daily$ID)) nrow=36 comes from 36 birds used
...
## Determine which bird is the least active in the Room 8 Dataset
...
tmp <- cbind(as.character(room8_daily$ID), as.numeric(room8_daily$total))
#length(unique(room8_daily$ID)) nrow=32 comes from 32 birds used
tmp2 <- matrix(as.vector(as.numeric(tmp[,2])), nrow=36, byrow=T)
total_totals <- rowSums(tmp2)
...
for(item in trans_reslist_room_8){
  room8_daily <- rbind(room8_daily, item$daily[c("sample","bottom","mid","top","total")])
  room8_weekly <- rbind(room8_weekly, item$weekly[c("sample","bottom","mid","top","total")])
  room8_monthly <- rbind(room8_monthly, item$monthly[c("sample","bottom","mid","top","total")])
}

room8_daily

most_active_intra_day <- room8_daily[grep(most_active,room8_daily$sample),]
...
p
ggsave(p, filename = "../figures/most_active_bird_pca_rm8_set_2.png",device = "png",width = unit(7,"in"),height=unit(7,"in"))
...
most_active_grp <- data.frame(x = most_active_intra_day$day, y = c(most_active_intra_day$bottom, most_active_intra_day$mid, most_active_intra_day$top), group= c(rep("Bottom",nrow(most_active_intra_day)),rep("Mid",nrow(most_active_intra_day)),rep("Top",nrow(most_active_intra_day))))
 ...
 q <- ggplot(most_active_grp, aes(x,y,col=group))+
  ...
  theme_bw()
q
ggsave(q, filename = "../figures/most_active_transitions_per_day_rm8_set_2.png",device="png")
```
...
least_active_intra_day <- room8_daily[grep(least_active,room8_daily$sample),]
...
p <- ggplot(pr.feb$x[,1:2], aes(x=PC1, y=PC2, color=as.character(least_active_intra_day$cluster)))+
  ...
  theme(legend.position = "bottom", legend.direction = "vertical",legend.text = element_text(size=8))
p
ggsave(p, filename = "../figures/least_active_bird_pca_rm8_set_2.png",device = "png",width = unit(7.5,"in"),height=unit(7.5,"in"))
...
q <-ggplot(least_active_grp, aes(x,y,col=group))+
  ...
  theme_bw()
q
ggsave(q, filename="../figures/least_active_transitions_per_day_rm8_set_2.png",device="png")
```
...
for(i in 1:length(trans_reslist_room_8)){
  res <- colSums(sapply(trans_reslist_room_8[[i]]$daily[,2:6], as.numeric))
  res[1] <- res[1]/length(trans_reslist_room_8[[i]]$daily$ID)
  res[6] <- round(res[2]/length(trans_reslist_room_8[[i]]$daily$bottom),0)
  res[7] <- round(res[3]/length(trans_reslist_room_8[[i]]$daily$mid),0)
  res[8] <- round(res[4]/length(trans_reslist_room_8[[i]]$daily$top),0)
  res[9] <- round(res[5]/length(trans_reslist_room_8[[i]]$daily$total),0)
  room8_daily_colsum <- rbind(room8_daily_colsum,res)
}

colnames(room8_daily_colsum) <- c("ID","bottom","mid","top","total","bottom_A","mid_A","top_a","total_A")
room8_daily_colsum
hold <- room8_daily_colsum[order(room8_daily_colsum$total_A,decreasing = T),]
...
cluster_table
write.csv(cluster_table, "../output/organized_birds_by_activity_daily_rm8_set_2.csv",row.names = F)
```
...
for(id in low_act_bird_ids){
  curr_bird <- room8_daily[grep(id,room8_daily$sample),]
...
q <-ggplot(low_act_grp, aes(x,y,col=group))+
  geom_line() +
  facet_grid(factor(group,levels=c("Top","Mid","Bottom"))~ .) +
  labs(title="Low Activity Birds Transitions for Room 8", color="Legend")+
  scale_x_continuous(name="Days",n.breaks = 14)+
...
ggsave(q, filename="../figures/low_active_transitions_rm8_set_2.png",device="png")
```
...
q <-ggplot(med_act_grp, aes(x,y,col=group))+
  geom_line() +
  facet_grid(factor(group,levels=c("Top","Mid","Bottom"))~ .) +
  labs(title="Medium Activity Birds Transitions Room 8", color="Legend")+
  scale_x_continuous(name="Days",n.breaks = 14)+
  scale_y_continuous(name="Transitions") +
  scale_color_manual(values=c("Top"="royalblue","Mid"="tomato","Bottom" = "seagreen")) +
  theme_bw()
...
ggsave(q, filename="../figures/med_active_transitions_rm8_set_2.png",device="png")
```
...
q <-ggplot(high_act_grp, aes(x,y,col=group))+
  geom_line() +
  facet_grid(factor(group,levels=c("Top","Mid","Bottom"))~ .) +
  labs(title="High Activity Birds Transitions in Room 8", color="Legend")+
  scale_x_continuous(name="Days",n.breaks = 14)+
  scale_y_continuous(name="Transitions") +
  scale_color_manual(values=c("Top"="royalblue","Mid"="tomato","Bottom" = "seagreen")) +
  theme_bw()
...
ggsave(q, filename="../figures/high_active_transitions_rm8_set_2.png",device="png")
```
...
room8_hourly <- data.frame()

#each item is one transition list for an intra bird sample
#we want to sum up the columns for each day and compare bird to bird

room8_hourly_colsum_min <- data.frame()

for(i in 1:length(trans_reslist_room8_min)){
  res <- colSums(sapply(trans_reslist_room8_min[[i]]$hour[,2:6], as.numeric))
  res[1] <- res[1]/length(trans_reslist_room8_min[[i]]$hour$ID)
  res[6] <- round(res[2]/length(trans_reslist_room8_min[[i]]$hour$bottom),0)
  res[7] <- round(res[3]/length(trans_reslist_room8_min[[i]]$hour$mid),0)
  res[8] <- round(res[4]/length(trans_reslist_room8_min[[i]]$hour$top),0)
  res[9] <- round(res[5]/length(trans_reslist_room8_min[[i]]$hour$total),0)
  room8_hourly_colsum_min <- rbind(room8_hourly_colsum_min,res)
}

colnames(room8_hourly_colsum_min) <- c("ID","bottom","mid","top","total","bottom_A","mid_A","top_a","total_A")
room8_hourly_colsum_min
hold_hour <- room8_hourly_colsum_min[order(room8_hourly_colsum_min$total,decreasing = T),]
...
minute_reslist_room8_hr <- c()

for(i in 1:length(reslist2_room_8)){
  minute_period <- to_minute(reslist2_room_8[[i]]$xts_obj$subzone)
  minute_reslist_room8_hr[[i]] <- minute_period
}

trans_reslist_room8_dur <- c()
for(i in 1:length(minute_reslist_room8_hr)){
  minute_trans_tab <- calc_trans_period_duration(reslist2_room_8[[i]]$ID, minute_reslist_room8_hr[[i]],reslist2_room_8[[i]]$xts_obj, "hr")
  #minute_trans_table <- calc_trans_period_duration(reslist2_room_8[[i]]$ID, minute_period, reslist2_room_8[[i]]$xts_obj, "Hr")
  result <- list("ID"=reslist2_room_8[[i]]$ID,"hour"=minute_trans_tab,"raw"=reslist2_room_8[[i]]$xts_obj)
  trans_reslist_room8_dur[[i]] <- result
}
...
trans_reslist_room8_dur[[1]]$ID

# Aggregate the high activity birds
example_duration <- na.trim(trans_reslist_room8_dur[[1]]$hour)
...
q <-ggplot(duration_hr_grp, aes(x,y,col=group))+
  geom_line() +
  facet_grid(factor(group,levels=c("Top","Mid","Bottom"))~ .) +
  labs(title="Example Birds Transition Duration in Room 8", color="Legend")+
...
ggsave(q, filename="../figures/duration_transitions_rm8_set_2.png",device="png")
ggsave(z, filename="../figures/duration_transitions_day_rm8_set_2.png",device="png")
```
