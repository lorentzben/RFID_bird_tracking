...
q <-ggplot(low_act_grp, aes(x,y,col=group))+
  ...
  scale_y_continuous(name="Average Transitions Into",n.breaks = 8,limits = c(0,65)) +
  scale_color_manual(values=c("Top"="royalblue","Mid"="tomato","Bottom" = "seagreen")) +
  theme_bw()
q
print("which birds are in this plot?")
print(low_act_bird_ids)
ggsave(q, filename="../figures/low_active_transitions_all_rooms.png",device="png")
```
...
ggsave(q, filename="../figures/med_active_transitions_all_rooms.png",device="png")
```
...
ggsave(q, filename="../figures/high_active_transitions_all_rooms.png",device="png")
```
...
for(i in 1:length(reslist2_room_2)){
  ...
  minute_trans_table <- calc_trans_period(reslist2_room_2[[i]]$ID, minute_period, reslist2_room_2[[i]]$xts_obj, "Hr",0)  
...
for(i in 1:length(reslist2_room_8)){
  minute_period <- to_minute(reslist2_room_8[[i]]$xts_obj$subzone)
  minute_trans_table <- calc_trans_period(reslist2_room_8[[i]]$ID, minute_period, reslist2_room_8[[i]]$xts_obj, "Hr",0)  
  result <- list("ID"=reslist2_room_8[[i]]$ID,"hour"=minute_trans_table, "raw"=reslist2_room_8[[i]]$xts_obj)
...
for(i in 1:length(minute_reslist_room2_hr)){
  minute_trans_tab <- calc_trans_period_duration(reslist2_room_2[[i]]$ID, minute_reslist_room2_hr[[i]],reslist2_room_2[[i]]$xts_obj, "hr",0)
  #minute_trans_table <- calc_trans_period_duration(reslist2_room_8[[i]]$ID, minute_period, reslist2_room_8[[i]]$xts_obj, "Hr")
...
for (j in 1:length(least_active))
{
  ...
  p <- ggplot(pr.feb$x[,1:2], aes(x=PC1, y=PC2, color=as.character(least_active_intra_day$cluster)))+
    geom_point() + 
    labs(title=paste("Least Active Bird (",least_active[i],")"),x=paste("PC1 (",round(pr.feb.prop$importance[[2,1]]*100,2),"%)"),y=paste("PC2 (",round(pr.feb.prop$importance[[2,2]]*100,2),"%)"),color="Cluster") +
    scale_color_discrete(name = "Weeks", labels= c(as.character(paste(least_active_intra_day$week[least_active_intra_day$cluster == 1],collapse=",")),as.character(paste(least_active_intra_day$week[least_active_intra_day$cluster == 2],collapse=",")),as.character(paste(least_active_intra_day$week[least_active_intra_day$cluster == 3],collapse=",")))) +
    theme_bw() + 
    theme(legend.position = "bottom", legend.direction = "vertical",legend.text = element_text(size=8))
  p
  ...
  q <-ggplot(least_active_grp, aes(x,y,col=group))+
    geom_line() +
    facet_grid(factor(group,levels=c("Top","Mid","Bottom"))~ .) +
    labs(title=paste("Least Active Bird (",least_active[i],") Transitions"), color="Legend")+
    scale_x_date(name="Weeks",breaks = seq(min(least_active_grp$x),max(least_active_grp$x),by="week"),labels = as.numeric(format(seq(min(least_active_grp$x),max(least_active_grp$x),by="week"),"%U"))+23)+
    scale_y_continuous(name="Transitions Into",n.breaks = 8,limits = c(0,70)) +
    scale_color_manual(values=c("Top"="royalblue","Mid"="tomato","Bottom" = "seagreen")) +
    theme_bw()
