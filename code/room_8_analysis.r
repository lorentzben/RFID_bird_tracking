...
room_8["DateTime"] <- as.POSIXct(room_8$access, origin="1970-01-01", tz="UTC")
...
for(i in 1:length(room_8_regular$tagname)){

    current_tag <- room_8_regular$tagname[i]

    current_tsibble <- room_8_regular |>
        slice(i) |> 
        pull(sampled) |> 
        pluck(1)
...
datebreaks <- seq(as.Date(as.POSIXct.numeric(head(unique(day_flat$interval1),n=1),origin="1970-01-01", tz="UTC")), as.Date(as.POSIXct.numeric(tail(unique(day_flat$interval1),n=1),origin="1970-01-01", tz="UTC")), by="7 days")
...
# TODO  Check Limits
room_8_sb_plot <- ggplot(data = day_flat, aes(x = as.Date(as.POSIXct.numeric(as.numeric(interval1),origin="1970-01-01", tz="UTC")), y=values, fill=ind)) + 
geom_bar(stat="identity") +
theme_bw() +  
xlab("Day of Study") + 
ylab("Prop. of Time Spent in Zone") +
...
# TODO  Check Limits
room_8_sb_night_plot <- ggplot(data = night_flat, aes(x = as.Date(as.POSIXct.numeric(as.numeric(interval1),origin="1970-01-01", tz="UTC")), y=values, fill=ind)) + 
geom_bar(stat="identity") +
theme_bw() +  
xlab("Day of Study") + 
ylab("Prop. of Time Spent in Zone") +
...
room_8_sb_day_plot <- ggplot(data = day_flat, aes(x = as.Date(as.POSIXct.numeric(as.numeric(interval1),origin="1970-01-01", tz="UTC")), y=values, fill=ind)) + 
geom_bar(stat="identity") +
theme_bw() +  
xlab("Day of Study") + 
ylab("Prop. of Time Spent in Zone") +
