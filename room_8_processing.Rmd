...
write.csv(room_8_daily,"room_8_daily_intra.csv",row.names = F)
write.csv(room_8_weekly,"room_8_weekly_intra.csv",row.names = F)
write.csv(room_8_monthly,"room_8_monthly_intra.csv",row.names = F)
...
p
ggsave(p, filename = "most_active_bird_pca_rm8.png",device = "png",width = unit(7,"in"),height=unit(7,"in"))
...
most_active_grp <- data.frame(x = most_active_intra_day$day, y = c(most_active_intra_day$bottom, most_active_intra_day$mid, most_active_intra_day$top), group= c(rep("Bottom",nrow(most_active_intra_day)),rep("Mid",nrow(most_active_intra_day)),rep("Top",nrow(most_active_intra_day))))
 ...
 q <- ggplot(most_active_grp, aes(x,y,col=group))+
  ...
  theme_bw()
q
ggsave(q, filename = "most_active_transitions_per_day_rm8.png",device="png")
```
...
p <- ggplot(pr.feb$x[,1:2], aes(x=PC1, y=PC2, color=as.character(least_active_intra_day$cluster)))+
  ...
  theme(legend.position = "bottom", legend.direction = "vertical",legend.text = element_text(size=8))
p
ggsave(p, filename = "least_active_bird_pca_rm8.png",device = "png",width = unit(7.5,"in"),height=unit(7.5,"in"))
...
q <-ggplot(least_active_grp, aes(x,y,col=group))+
  ...
  theme_bw()
q
ggsave(q, filename="least_active_transitions_per_day_rm8.png",device="png")
```
...
cluster_table
write.csv(cluster_table, "organized_birds_by_activity_daily_rm8.csv",row.names = F)
```
...
ggsave(q, filename="low_active_transitions_rm8.png",device="png")
```
...
ggsave(q, filename="med_active_transitions_rm8.png",device="png")
```
...
ggsave(q, filename="high_active_transitions_rm8.png",device="png")
```
...
ggsave(q, filename="duration_transitions_rm8.png",device="png")
ggsave(z, filename="duration_transitions_day_rm8.png",device="png")
```
