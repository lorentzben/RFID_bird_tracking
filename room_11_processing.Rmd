...
for(i in 1:length(bird_ids_6894_new)){
  res <- sep_bird_id_period(samp_name="6894",cage_obj =six_eight_nine_four, samp_id = bird_ids_6894_new[i],cutoff="2020-12-02 T23:00:00/2021-02-18 T10:00:00")
  reslist2_6894[[i]] <- res
}
...
whole_data_sanity <- reslist2_6894[[1]]$daily_obj
...
write.csv(room_11_daily,"room_11_daily_intra.csv",row.names = F)
write.csv(room_11_weekly,"room_11_weekly_intra.csv",row.names = F)
write.csv(room_11_monthly,"room_11_monthly_intra.csv",row.names = F)
...
for(i in 1:length(reslist2_6894)){
  minute_period <- to_minute(reslist2_6894[[i]]$xts_obj$subzone)
  minute_trans_table <- calc_trans_period(reslist2_6894[[i]]$ID, minute_period, reslist2_6894[[i]]$xts_obj, "Hr")  
  result <- list("ID"=reslist2_6894[[i]]$ID,"hour"=minute_trans_table, "raw"=reslist2_6894[[i]]$xts_obj)
...
tmp2 <- matrix(as.vector(as.numeric(tmp[,2])), nrow=7, byrow=T)
...
p
ggsave(p, filename = "most_active_bird_pca_rm11.png",device = "png",width = unit(7,"in"),height=unit(7,"in"))
...
most_active_grp <- data.frame(x = most_active_intra_day$day, y = c(most_active_intra_day$bottom, most_active_intra_day$mid, most_active_intra_day$top), group= c(rep("Bottom",nrow(most_active_intra_day)),rep("Mid",nrow(most_active_intra_day)),rep("Top",nrow(most_active_intra_day))))
 ...
 q <- ggplot(most_active_grp, aes(x,y,col=group))+
  ...
  theme_bw()
q
ggsave(q, filename = "most_active_transitions_per_day_rm11.png",device="png")
```
...
p <- ggplot(pr.feb$x[,1:2], aes(x=PC1, y=PC2, color=as.character(least_active_intra_day$cluster)))+
  ...
  theme(legend.position = "bottom", legend.direction = "vertical",legend.text = element_text(size=8))
p
ggsave(p, filename = "least_active_bird_pca_rm11.png",device = "png",width = unit(7.5,"in"),height=unit(7.5,"in"))
...
q <-ggplot(least_active_grp, aes(x,y,col=group))+
  ...
  theme_bw()
q
ggsave(q, filename="least_active_transitions_per_day_rm11.png",device="png")
```
...
cluster_table
write.csv(cluster_table, "organized_birds_by_activity_daily_rm11.csv",row.names = F)
```
...
ggsave(q, filename="low_active_transitions_rm11.png",device="png")
```
...
ggsave(q, filename="med_active_transitions_rm11.png",device="png")
```
...
ggsave(q, filename="high_active_transitions_rm11.png",device="png")
```
...
for(i in 1:length(minute_reslist_room11_hr)){
  minute_trans_tab <- calc_trans_period_duration(reslist2_6894[[i]]$ID, minute_reslist_room11_hr[[i]],reslist2_6894[[i]]$xts_obj, "hr")
  #minute_trans_table <- calc_trans_period_duration(reslist2_6928[[i]]$ID, minute_period, reslist2_6928[[i]]$xts_obj, "Hr")
  result <- list("ID"=reslist2_6894[[i]]$ID,"hour"=minute_trans_tab,"raw"=reslist2_6894[[i]]$xts_obj)
...
ggsave(q, filename="duration_transitions_rm11.png",device="png")
ggsave(z, filename="duration_transitions_day_rm11.png",device="png")
```
