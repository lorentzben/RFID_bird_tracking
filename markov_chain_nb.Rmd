...
dec_to_feb <- reslist[c(1:10)]

for(item in dec_to_feb){
  item$xts_obj <- item$xts_obj["2020-12-02 23:00:00/2021-01-22 10:00:00"]
  #item$daily_obj <- to.daily(item$xts_obj, OHLC=F)
  #item$weekly_obj <- to.weekly(item$xts_obj, OHLC=F)
  #item$monthly_obj <- to.monthly(item$xts_obj, OHLC=F)
}

for(item in dec_to_feb){
  print(item$ID)
  print(summary(item$xts_obj))
  print(summary(item$daily_obj))
}
