...
```{r calculate time in zone for each day of study}
  ...
  calc_zone_duration_daily <- function(bird_id, bird_trans){
    ...
    first_day <- as.Date(dmy_hms(bird_trans$accessdate[1]))
    last_day <- as.Date(tail(dmy_hms(bird_trans$accessdate),n=1))
    ...
    for(i in 1:length(series)){
      ...
      daily_sum_table <- rbind(daily_sum_table,calc_zone_durationseries[i], bird_id, one_day))
    }
